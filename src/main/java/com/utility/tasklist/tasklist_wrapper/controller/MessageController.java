...
public class MessageController {
     ...
     * Exposed at: POST /process-instances/{processInstanceKey}/migration
     */
    @PostMapping("/process-instances/{processInstanceKey}/migration")
    public ResponseEntity<?> migrateProcess(@PathVariable String processInstanceKey, @RequestBody MigrateProcessInstanceRequest migrationRequestBody) {
        try {
            Object result = camundaTaskService.migrateProcessInstance(processInstanceKey, migrationRequestBody);
            return ResponseEntity.ok(result);
        } catch (Exception ex) {
            return ResponseEntity.status(500).body("Failed to migrate process: " + ex.getMessage());
        }
    }
